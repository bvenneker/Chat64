...
#define oC64D0 GPIO_NUM_19  // data bit 0 for data from the ESP32 to the C64
#define oC64D1 GPIO_NUM_18  // data bit 1 foswitch data fswitchom the ESP32 to the C64
#define oC64D2 GPIO_NUM_26  // data bit 2 for data from the ESP32 to the C64
#define oC64D3 GPIO_NUM_13  // data bit 3 for data from the ESP32 to the C64
#define oC64D4 GPIO_NUM_17  // data bit 4 for data from the ESP32 to the C64
#define oC64D5 GPIO_NUM_12  // data bit 5 for data from the ESP32 to the C64
#define oC64D6 GPIO_NUM_23  // data bit 6 for data from the ESP32 to the C64
#define oC64D7 GPIO_NUM_27  // data bit 7 for data from the ESP32 to the C64

#define oC64RST GPIO_NUM_2   // reset signal to C64
#define oC64NMI GPIO_NUM_21  // non-maskable interrupt signal to C64
#define CLED GPIO_NUM_25     // led on cartridge

// ********************************
// **        INPUTS             **
// ********************************
#define resetSwitch GPIO_NUM_15  // this pin outputs PWM signal at boot
#define C64IO1 GPIO_NUM_16
#define C64IO2 GPIO_NUM_33
#define iC64D0 GPIO_NUM_5
#define iC64D1 GPIO_NUM_34
#define iC64D2 GPIO_NUM_36
#define iC64D3 GPIO_NUM_35
#define iC64D4 GPIO_NUM_22
#define iC64D5 GPIO_NUM_32
#define iC64D6 GPIO_NUM_4
#define iC64D7 GPIO_NUM_14
...
void IRAM_ATTR isr_io1() {
  ...
  ch |= gpio_get_level(iC64D7) << 7;
  ch |= gpio_get_level(iC64D6) << 6;
  ch |= gpio_get_level(iC64D5) << 5;
  ch |= gpio_get_level(iC64D4) << 4;
  ch |= gpio_get_level(iC64D3) << 3;
  ch |= gpio_get_level(iC64D2) << 2;
  ch |= gpio_get_level(iC64D1) << 1;
  ch |= gpio_get_level(iC64D0) << 0;
  dataFromC64 = true;
}
...
void setup() {
  ...
  // define inputs
  pinMode(iC64D0, INPUT);
  pinMode(iC64D1, INPUT);
  pinMode(iC64D2, INPUT);
  pinMode(iC64D3, INPUT);
  pinMode(iC64D4, INPUT);
  pinMode(iC64D5, INPUT);
  pinMode(iC64D6, INPUT);
  pinMode(iC64D7, INPUT);
  pinMode(C64IO1, INPUT);
  pinMode(C64IO2, INPUT);
  pinMode(resetSwitch, INPUT_PULLUP);

  // define interrupts
  attachInterrupt(C64IO1, isr_io1, FALLING);         // interrupt for io1, C64 writes data to io1 address space
...
void Task1code(void* parameter) {
  ...
  for (;;) {  // this is an endless loop
    ...
    while (getMessage == false) {           // this is a wait loop
      ...
      if (fullpage == true and WiFi.status() == WL_CONNECTED and serverconnected) getMessage = true;
    }
    ...
    if (httpResponseCode == 200) {           // httpResponseCode should be 200
      ...
      msgbuffersize = textOutput.length() + 1;  //
      SerialDebug += "Buffer size =" + (String)msgbuffersize;
      if (msgbuffersize > 498) {  // that should never happen
        msgbuffersize = 498;
        SerialDebug += "\nError: msgbuffer is too large!\n" + textOutput;
      }
      ...
      if (!error) {
        ...
        if (newid) {
#ifdef debug
          SerialDebug += "\nnew lastmessage id=" + (String)tempMessageID;
#endif
...
#ifdef debug
        if (textOutput != "") SerialDebug += "\nError: Json deserialize error in getmessage";
#endif
      }
    } else {
#ifdef debug
      SerialDebug += "\nError: Readmessage response = " + (String)httpResponseCode;
#endif
    }
    // Free resources
    http.end();
...
void loop() {
  ...
  if (dataFromC64) {
    ...
    digitalWrite(CLED, wificonnected);
