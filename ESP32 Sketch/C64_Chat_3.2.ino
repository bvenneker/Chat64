#include "WiFi.h"           // this library is part of the esp32 installation
#include "HTTPClient.h"     // this library is part of the esp32 installation
...
void setup() {
  ...
  // define outputs
  pinMode(CLED, OUTPUT); digitalWrite(CLED, LOW);
  ...
  pinMode(oC64D7, OUTPUT);  digitalWrite(oC64D7, LOW);
  pinMode(oC64RST, OUTPUT); digitalWrite(oC64RST, invert_Reset);
  pinMode(oC64NMI, OUTPUT); digitalWrite(oC64NMI, LOW);
  pinMode (pload,OUTPUT);   digitalWrite(pload,LOW);   // must be low to load parallel data
  pinMode(sclk,OUTPUT); 
...
void Task1code( void * parameter) {  
  ...
  for(;;) {                                       // this is an endless loop
    ...
    while (getMessage==false)  {                  // this is a wait loop
      ...
      if (firstContact and millis() > heartbeat+25000) {           // while we do nothing we send a heartbeat signal to the server 
        heartbeat=millis();                       // so that the web server knows you are still on line
        SendMessageToServer("","","heartbeat");   // heartbeat repeats every 25 seconds
        }            
      }
    ...
    if (httpResponseCode == 200) {                                        // httpResponseCode should be 200
      ...
      #ifdef debug
        Serial.print("Message type (1=private) = ");
        Serial.println(isPrivateMsg); 
      ...
      if (!error) {
        ...
        if ((isPrivateMsg and newMessageId !=lastprivmsg) or (!isPrivateMsg and newMessageId != lastmessage)){
          if (isPrivateMsg) {
            String nickname= doc["nickname"];
            if ( nickname!= myNickName){ pmSender = "@" + nickname;}  
          }

          tempMsgId = newMessageId;
...
String getUserList(int page){
  ...
  String httpRequestData = "regid=" + regID + "&page=" + page + "&call=" + call;
  int httpResponseCode = http.POST(httpRequestData);  
  String result="0";
...
// *************************************************
void SendMessageToServer(String Encoded, String RecipientName, String call) {
  ...
  // Prepare your HTTP POST request data
  String httpRequestData = "sendername=" + myNickName +"&call=" + call + "&regid=" + regID + "&recipientname=" + RecipientName + "&message=" + Encoded +  "";
...
void loop() {
  ...
  if (dataFromC64) {
    ...
    if (ch == 253) {
      ...
      SendMessageToServer(Encoded, RecipientName,"insert");
    } 
